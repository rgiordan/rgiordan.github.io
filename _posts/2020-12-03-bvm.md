...
data $$x=(x_1,\ldots,x_N)$$, a parametric model $$p(x_n | \theta)$$, and a prior
...
where the Taylor series residuals are controlled by the integral form of the
remainder rather than the mean value theorem.  To avoid tedious notation, we'll
just focus on the scalar case.
...
the form $$\int \phi(\theta) \exp(\sum_{n=1}^N \ell(x_n | \theta))p(\theta)
d\theta$$. In general, $$p(\theta)$$ and $$\phi(\theta)$$ are quite nonzero over
...
essentially the same.  It is the rest of the domain (typically a set with prior
